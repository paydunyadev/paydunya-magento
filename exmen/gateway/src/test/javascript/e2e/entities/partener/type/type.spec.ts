...
describe('Type e2e test', () => {
  ...
  it('should load Types', async () => {
    ...
    expect(await typeComponentsPage.getTitle()).to.eq('gatewayApp.partenerType.home.title');
  });

  it('should load create Type page', async () => {
    await typeComponentsPage.clickOnCreateButton();
    typeUpdatePage = new TypeUpdatePage();
    expect(await typeUpdatePage.getPageTitle()).to.eq('gatewayApp.partenerType.home.createOrEditLabel');
  ...
  it('should delete last Type', async () => {
    ...
    expect(await typeDeleteDialog.getDialogTitle()).to.eq('gatewayApp.partenerType.delete.question');
